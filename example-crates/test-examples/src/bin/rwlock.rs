...
async fn foo() {
    ...
    join!(
        task(
            Task::new("r1").with_start_barrier(barrier.clone()),
            do_read(var.clone(), rwlock_id.clone())
        ),
        task(
            Task::new("r2").with_start_barrier(barrier.clone()),
            do_read(var.clone(), rwlock_id.clone())
        ),
        task(
            Task::new("w1").with_start_barrier(barrier.clone()),
            do_write(var.clone(), rwlock_id.clone())
        ),
        task(
            Task::new("w2").with_start_barrier(barrier.clone()),
            do_write(var.clone(), rwlock_id.clone())
        ),
    );
